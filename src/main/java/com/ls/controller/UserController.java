...
public class UserController {
    ...
    public BaseResponse<Boolean> addSignin(HttpServletRequest request) {
        User loginUser = userService.getLoginUser(request);
        return ResultUtils.success(userService.addUserSign(loginUser.getId()));
    }
     ...
     * @param year    年
     * @param request 请求
     * @return {@link BaseResponse }<{@link Map }<{@link LocalDate }, {@link Boolean }>>
     */
    @GetMapping("/get/signin")
    public BaseResponse<List<Integer>> getUserSignin(Integer year, HttpServletRequest request) {
        User loginUser = userService.getLoginUser(request);
        return ResultUtils.success(userService.getUserSignin(year, loginUser.getId()));
    }
