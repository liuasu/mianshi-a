...
public class QuestionServiceImpl extends ServiceImpl<QuestionMapper, Question> implements QuestionService {
    ...
    @Resource
    private QuestionBankQuestionService qbqService;
    ...
    public Page<Question> listQuestionByPage(QuestionQueryRequest questionQueryRequest) {
        ...
        if (questionBankId != null && questionBankId > 0) {
            List<QuestionBankQuestion> qbqionList = qbqService.lambdaQuery()
                    .select(QuestionBankQuestion::getQuestionId)
                    .eq(QuestionBankQuestion::getQuestionBankId, questionBankId)
                    .list();
            if (CollUtil.isNotEmpty(qbqionList)) {
                List<Long> questionIds = qbqionList.stream().map(QuestionBankQuestion::getQuestionId).collect(Collectors.toList());
    ...
    public Page<Question> searchFromEs(QuestionQueryRequest questionQueryRequest) {
        ...
        NativeSearchQuery searchQuery = new NativeSearchQueryBuilder()
                ...
                .build();
        SearchHits<QuestionEsDTO> searchHits = estTemplate.search(searchQuery, QuestionEsDTO.class);
    ...
    public void batchDelQuestions(QuestionBatchDelRequest questionBatchDelRequest) {
        List<Long> questionIdList = questionBatchDelRequest.getQuestionIdList();
        ...
        for (Long questionId : questionIdList) {
            ...
            res = qbqService.lambdaUpdate()
                    .eq(QuestionBankQuestion::getQuestionId, questionId)
                    .remove();
            ThrowUtils.throwIf(!res, ErrorCode.OPERATION_ERROR, "题库关系删除失败");
        }
    }
}
